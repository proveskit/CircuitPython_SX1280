...
class SX1280:
    ...
    def _readRegisters(self, address1, address2, _length=1):
        ...
        self._busywait()
        with self._device as device:
            device.write_readinto(
                bytes([_RADIO_READ_REGISTER, address1, address2, 0] + [0] * _length),
                self._BUFFER,
                out_end=_size,
                in_end=_size,
            )
    ...
    def printRegisters(self, start=0x0900, stop=0x09FF, p=True):
        ...
        with self._device as device:
            device.write_readinto(
                bytes([_RADIO_READ_REGISTER])
                + int(start).to_bytes(2, "big")
                + b"\x00"
                + b"\x00" * _length,
                buff,
                out_end=_size,
    ...
    def sleep(self):
        ...
        with self._device as device:
            device.write_readinto(
                bytes([_RADIO_SET_SLEEP, 0x07]), self._BUFFER, out_end=2, in_end=2
            )
        self.sleeping = True
        if self._debug:
    ...
    def status(self, raw=False):
        ...
        with self._device as device:
            device.write_readinto(
                bytes([_RADIO_GET_STATUS]), self._BUFFER, out_end=1, in_end=1
            )
                ...
                if ack_packet is not None:
                    if ack_packet[3] & _RH_FLAGS_ACK:
                        ...
                        if ack_packet[2] == self.identifier:
                            ...
                            print(
                                "bad sband ack ID. Looking for: {}".format(
                                    hex(self.identifier)
                                )
                            )
                ...
                if self.default_dio.value:
                    ...
                    if irq:
                        ...
                        if "RngSlaveResponseDone" in irq:
                            ...
                            self.clear_Irq_Status()
                            if debug:
                                print("[range slave] responded to range request")
                            return True
    ...
    def send_fast(self, data, length):
        self.txen.value = True
        self.pcktparams["PayloadLength"] = length
        self.set_Packet_Params()
        self._busywait()
        with self._device as device:
            device.write(b"\x1a\x00" + data, end=length + 2)
        self.set_Tx()
        txdone = self.wait_for_irq()
        self.txen.value = False
        return txdone
